...
describe('Guitar Pro tablature', () => {
  ...
  it('keeps open-string pitches with a capo and nonstandard tuning', async () => {
    ...
    expect(restored.getStaffTunings().map((tuning) => tuning.midi)).toEqual([55, 60]);
    expect(restored.getStaffDetails()?.capo).toBe(2);
    expect(restored.notes.map((note) => [note.string, note.fret, note.pitch?.step, note.pitch?.octave])).toEqual([
      [1, 0, 'D', 4],
      [2, 0, 'A', 3],
    ]);
  ...
  it('writes GPIF string indices and frets independently of the mdom reader', async () => {
    const note = measure.getOrCreateVoice('1').addNote({ step: 'E', octave: 4, type: 'whole' });
    note.setStringFret({ string: 1, fret: 2 });
    const zip = await JSZip.loadAsync(await serializer.serializeToBytes(document));
    const gpif = new MDOMParser().parseFromString(await zip.file('Content/score.gpif')!.async('string')).root;
  ...
  it('rejects a fret that disagrees with the pitch', async () => {
    const note = measure.getOrCreateVoice('1').addNote({ step: 'D', octave: 4, type: 'whole' });
    note.setStringFret({ string: 1, fret: 4 });

    await expect(serializer.serializeToBlob(document)).rejects.toThrow('pitch disagrees');
  });
  ...
  it('rejects a string outside the tuning', async () => {
    ...
    await expect(serializer.serializeToBlob(document)).rejects.toThrow('Invalid string');
  });

  it('rejects a missing string assignment on a tablature staff', async () => {
    measure.getOrCreateVoice('1').addNote({ step: 'D', octave: 4, type: 'whole' });

    await expect(serializer.serializeToBlob(document)).rejects.toThrow('needs a string and fret');
  });
  ...
  it('rejects string assignments without a tuning', async () => {
    ...
    await expect(serializer.serializeToBlob(document)).rejects.toThrow('requires tuning, string and fret');
  });

  it('rejects a tuning with missing line numbers', async () => {
    measure.getStaffTunings()[0]!.setAttribute('line', '3');

    await expect(serializer.serializeToBlob(document)).rejects.toThrow('tuning lines must be consecutive');
  });
  ...
  it('rejects a capo change after the first measure', async () => {
    ...
    await expect(serializer.serializeToBlob(document)).rejects.toThrow('mid-score tuning or capo changes');
  });
});
