...
export class Measure extends MElement {
  ...
  // Position among the part's measures; -1 when detached. Distinct from `number`,
  // which is the (free-form, possibly repeated) printed label.
  get index(): number {
    return this.closest(Part)?.measures.indexOf(this) ?? -1;
  }

  // The <clef>/<key>/<time> in effect at the start of this measure for `staff`
  // (default '1'): the nearest declaration at or before this measure. Clefs match
  // their staff exactly; a numberless key/time applies to every staff.
  ...
  // Upsert the <clef>/<key>/<time> in this measure's <attributes> (created and
  // positioned first if absent). The caller never assembles <attributes> by hand.
