...
describe('editing a score by intent — CRUD', () => {
  ...
  it('computes <duration> from the note type and dots', () => {
    const voice = MDocument.empty().score!.addPart({ id: 'P1' }).addMeasure().voice('1');
    const dotted = voice.note({ step: 'C', octave: 4, type: 'quarter', dots: 1 });
  ...
  it('lays a second voice on its own staff, inserting the <backup> itself', () => {
    ...
    measure.voice('2', { staff: '2' }).note({ step: 'C', octave: 3, type: 'whole' });
  ...
  it('keeps a sibling voice anchored by repairing the handoff <backup>', () => {
    ...
    const bass = measure.voice('2', { staff: '2' }).note({ step: 'C', octave: 3, type: 'half' });
  ...
  it('cuts one link of a tie chain, leaving the rest tied', () => {
    ...
    first.tieTo(middle);
    middle.tieTo(last); // the middle note now holds both ends

    middle.untieFrom(first); // name which one to cut
  ...
  it('writes a guitar octave clef (treble-8)', () => {
    ...
    expect(measure.clef('1')!.octaveChange).toBe(-1);
  });

  it('frets a note on a string with setStringFret (a tab note keeps its pitch)', () => {
    const voice = MDocument.empty().score!.addPart({ id: 'P1', name: 'Guitar' }).addMeasure().voice('1');
    const note = voice.note({ step: 'E', octave: 2, type: 'quarter' });
  ...
  it('round-trips a tab note through MusicXML', () => {
    ...
    const note = doc
      .score!.addPart({ id: 'P1', name: 'Guitar' })
      .addMeasure()
      .voice('1')
      .note({ step: 'A', octave: 2, type: 'quarter' });
    note.setStringFret({ string: 5, fret: 0 });

    const xml = new MusicXMLSerializer().serializeToString(doc);
    const read = new MDOMParser().parseFromString(xml).score!.part('P1')!.measure('1')!.notes[0]!;
