...
describe('editing a score by intent — CRUD', () => {
	...
	it('stacks a chord on one onset', () => {
		...
		expect(triad.notes.map((note) => note.isChordMember)).toEqual([
			false,
			true,
			true,
		]);
	...
	it('lays a second voice on its own staff, inserting the <backup> itself', () => {
		const measure = MDocument.empty()
			.score.addPart({ id: 'P1', name: 'Piano' })
			.addMeasure();
		...
		expect(measure.notes.map((note) => note.staff)).toEqual([
			'1',
			'1',
			'1',
			'2',
	...
	it('keeps a sibling voice anchored by repairing the handoff <backup>', () => {
		...
		const bass = measure
			.getOrCreateVoice('2', { staff: '2' })
			.addNote({ step: 'C', octave: 3, type: 'half' });
