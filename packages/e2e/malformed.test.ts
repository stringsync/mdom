...
const loadExample = (file: string): string =>
	fs.readFileSync(path.join(__dirname, 'examples', file), 'utf-8');

const roundTrip = (xml: string): string =>
	serializer.serializeToString(parser.parseFromString(xml));

describe('malformed input', () => {
	it('throws on input that is not well-formed XML', () => {
		expect(() =>
			parser.parseFromString(loadExample(MALFORMED.INVALID_ROOT)),
		).toThrow();
	});

	it('parses valid-but-empty MusicXML to a score with no parts', () => {
		const score = parser.parseFromString(
			loadExample(MALFORMED.MOSTLY_INVALID),
		).score;
