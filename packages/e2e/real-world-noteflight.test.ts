...
describe('Noteflight — navigation landmarks and tempo', () => {
	...
	it('reads the segno and coda a D.S. jumps to', () => {
		expect(directions.flatMap((direction) => direction.navigations)).toEqual([
			'segno',
			'coda',
		]);
	});
	...
	it('reads the jump instructions as words', () => {
		expect(directions.flatMap((direction) => direction.words)).toEqual([
			'to coda',
			'D.S. al Coda',
		]);
	});
