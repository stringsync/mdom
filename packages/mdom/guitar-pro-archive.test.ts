...
describe('Guitar Pro archives', () => {
  ...
  it('rejects legacy binary formats with an actionable error', async () => {
    await expect(parser.parseFromBytes(new TextEncoder().encode('FICHIER GUITAR PRO v5.10'))).rejects.toThrow(
      '.gp3, .gp4, .gp5 and .gpx are not supported'
    );
  });
  ...
  it('rejects an unrelated ZIP archive', async () => {
    ...
    await expect(parser.parseFromBytes(await zip.generateAsync({ type: 'uint8array' }))).rejects.toThrow(
      'Content/score.gpif'
    );
  });

  it('rejects a truncated ZIP archive', async () => {
    await expect(parser.parseFromBytes(new Uint8Array([0x50, 0x4b, 3, 4]))).rejects.toThrow();
  });
  ...
  it('packages a score with version and layout entries', async () => {
    ...
    document.score.addPart().addMeasure().getOrCreateVoice('1').addNote({ step: 'C', octave: 4, type: 'whole' });
    ...
    expect(new MDOMParser().parseFromString(await zip.file('Content/score.gpif')!.async('string')).root.tag).toBe(
      'GPIF'
    );
  });

  it('rejects a document without music', async () => {
    await expect(serializer.serializeToBlob(MDocument.empty())).rejects.toThrow('at least one part with measures');
  });
});
