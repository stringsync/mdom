...
describe('metronome — both notated forms, read whole', () => {
	...
	it('splits the note-group form at <metronome-relation>', () => {
		...
		expect(relation.left.map((note) => note.type)).toEqual([
			'eighth',
			'eighth',
		]);
		expect(relation.left.map((note) => note.beams)).toEqual([
			['begin'],
			['end'],
		]);
		...
		expect(relation.right[0]!.tuplet).toEqual({
			type: 'start',
			actual: 3,
			normal: 2,
		});
	...
	it('keeps the old flattened accessor working off the first unit', () => {
		expect(rate!.metronome).toEqual({
			beatUnit: 'quarter',
			dots: 0,
			perMinute: 'ca. 120',
		});
		expect(paired!.metronome).toEqual({
			beatUnit: 'quarter',
			dots: 1,
			perMinute: null,
		});
