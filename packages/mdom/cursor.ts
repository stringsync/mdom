...
export class Cursor {
	...
	static at(target: Note | Voice, beat = 0): Cursor {
		if (target instanceof Note) {
			const measure = required(
				target.closest(Measure),
				'<measure> ancestor for cursor',
			);
	...
	get note(): Note | null {
		...
		return (
			this.stopsIn(this.measure).find(
				(note) => note.measureBeat === this.onset,
			) ?? null
		);
	}
	 ...
	 */
	next(): Cursor | null {
		const later = this.stopsIn(this.measure).find(
			(note) => (note.measureBeat ?? 0) > this.onset,
		);
		if (later) {
	 ...
	 */
	prev(): Cursor | null {
		const earlier = this.stopsIn(this.measure).filter(
			(note) => (note.measureBeat ?? 0) < this.onset,
		);
		if (earlier.length > 0) {
	...
	private stopsIn(measure: Measure): Note[] {
		return measure
			.getOrCreateVoice(this.voiceId, { staff: this.staff })
			.chords.map((chord) => chord.notes[0]!);
	}
	...
	private measuresFrom(step: 1 | -1): Measure[] {
		...
		return step > 0
			...
			: measures.slice(0, index).reverse();
	}
}
