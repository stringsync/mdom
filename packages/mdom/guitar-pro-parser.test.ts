...
describe('GuitarProParser', () => {
	...
	it('reads pitches, rests and durations from an independent Guitar Pro file', async () => {
		const doc = await parser.parseFromBlob(
			Bun.file(new URL('./fixtures/guitar-pro/notes.gp', import.meta.url)),
		);
	...
	it('keeps string numbering, frets and tuning from an independent chord fixture', async () => {
		...
		expect(
			measure.notes
				.slice(0, 6)
				.map((note) => [note.string, note.fret, note.measureBeat]),
		).toEqual([
			[1, 1, 0],
			[2, 2, 0],
			[3, 3, 0],
		...
		expect(
			measure.getStaffTunings().map((tuning) => [tuning.line, tuning.midi]),
		).toEqual([
			[1, 40],
			[2, 45],
			[3, 50],
		...
		expect(measure.notes.at(-1)).toMatchObject({
			isRest: true,
			beats: 3,
			dots: 1,
		});
	...
	it('keeps tuplet ratios and exact onsets from an independent fixture', async () => {
		...
		expect(measure.notes.map((note) => note.measureBeat)).toEqual([
			0,
			2 / 3,
			4 / 3,
			2,
		...
		expect(
			measure.notes.slice(0, 3).map((note) => note.timeModification),
		).toEqual([
			{ actual: 3, normal: 2 },
			{ actual: 3, normal: 2 },
			{ actual: 3, normal: 2 },
		]);
		expect(measure.notes.map((note) => note.duration)).toEqual([
			512, 512, 512, 1536,
		]);
		expect(schemaErrors(doc)).toEqual([]);
	});
	...
	it('reads score metadata and multiple tracks', async () => {
		const doc = await parser.parseFromBlob(
			Bun.file(new URL('./fixtures/guitar-pro/score-info.gp', import.meta.url)),
		);

		expect(doc.score.title).toBe('Title');
		expect(doc.score.parts.map((part) => part.label)).toEqual([
			'Track 1',
			'Track 2',
		]);
		expect(
	...
	it('reads changing meters without flattening the denominator', async () => {
		const doc = await parser.parseFromBlob(
			Bun.file(
				new URL('./fixtures/guitar-pro/time-signatures.gp', import.meta.url),
			),
		);

		expect(
			doc.score.parts[0]!.measures.map((measure) => [
				measure.getTime()?.beats,
				measure.getTime()?.beatType,
			]),
		).toEqual([
			['4', '4'],
			['3', '4'],
	...
	it('reads major and minor key changes', async () => {
		const doc = await parser.parseFromBlob(
			Bun.file(
				new URL('./fixtures/guitar-pro/key-signatures.gp', import.meta.url),
			),
		);
		...
		expect(
			measures
				.slice(0, 4)
				.map((measure) => [measure.getKey()?.fifths, measure.getKey()?.mode]),
		).toEqual([
			[0, 'major'],
			[-1, 'major'],
			[-2, 'major'],
	...
	it('keeps pickup measures short', async () => {
		const doc = await parser.parseFromBlob(
			Bun.file(new URL('./fixtures/guitar-pro/anacrusis.gp', import.meta.url)),
		);
	...
	it('reads an empty GP8 bar as a full-measure rest', async () => {
		const doc = await parser.parseFromBlob(
			Bun.file(
				new URL('./fixtures/guitar-pro/hide-tuning.gp', import.meta.url),
			),
		);
	...
	it('reads repeat counts from an independent fixture', async () => {
		const doc = await parser.parseFromBlob(
			Bun.file(
				new URL('./fixtures/guitar-pro/repeat-close.gp', import.meta.url),
			),
		);

		expect(
			doc.score.parts[0]!.measures.flatMap((measure) =>
				measure.barlines.map((line) => [line.repeat, line.repeatTimes]),
			),
		).toEqual([
			['forward', null],
			['backward', 2],
	...
	it('retains grace timing when unsupported techniques are explicitly omitted', async () => {
		...
		expect(notes.map((note) => note.isGrace)).toEqual([
			true,
			false,
			true,
			false,
		...
		expect(
			notes.filter((note) => note.isGrace).map((note) => note.duration),
		).toEqual([null, null]);
		expect(schemaErrors(doc)).toEqual([]);
	});
});
