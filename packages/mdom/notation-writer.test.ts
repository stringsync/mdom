...
const serialize = (doc: MDocument): string =>
...
describe('measure and note — writing common notation', () => {
	it('writes a chord symbol with a fretboard diagram', () => {
		...
		measure
			.getOrCreateVoice('1')
			.addNote({ step: 'B', octave: 3, type: 'whole' });
		...
		expect(
			harmony.frame?.frameNotes.map((note) => [
				note.string,
				note.fret,
				note.barre,
			]),
	...
	it('writes a dotted metronome mark, its playback tempo, and an expression', () => {
		...
		const dynamic = measure.addDirection({
			dynamics: ['sf', 'p'],
			placement: 'below',
		});
		measure
		...
		expect(tempo.metronomes[0]!.beatUnits).toEqual([
			{ type: 'quarter', dots: 1 },
		]);
	...
	it('writes articulations, gathering them into one <articulations> block', () => {
		...
		expect(
			note.childrenNamed('notations')[0]!.childrenNamed('articulations'),
		).toHaveLength(1);
		expect(schemaErrors(doc)).toEqual([]);
	});
	...
	it('upserts a diagram onto a harmony that already has one', () => {
		...
		measure
			.getOrCreateVoice('1')
			.addNote({ step: 'E', octave: 3, type: 'whole' });
