...
import { directionMarkers, noteMarkers, Spanner } from './spanner';
...
describe('spanner — the generic start/stop resolver', () => {
	...
	const step = (slur: Slur | null): string | null =>
		slur?.note?.pitch?.step ?? null;
	...
	it('collects note-attached markers across the part in document order', () => {
		...
		const types = noteMarkers(firstSlur, (note) => note.slurs).map(
			(slur) => slur.slurType,
		);
	...
	it('returns the whole start..stop run from any member', () => {
		...
		expect(members.map((slur) => slur.slurType)).toEqual([
			'start',
			'continue',
			'stop',
		]);
	...
	it('returns the whole run from a middle member via spanOpener', () => {
		...
		expect(spanner.membersOf(continueE5).map((slur) => step(slur))).toEqual([
			'D',
			'E',
			'F',
		]);
		expect(spanner.membersOf(stopF5).map((slur) => step(slur))).toEqual([
			'D',
			'E',
			'F',
		]);
	...
	it('collects direction-attached markers from <direction>s', () => {
		...
		const firstWedge = part
			.getMeasure('1')!
			.directions.flatMap((direction) => direction.wedges)[0]!;
		const wedges = directionMarkers(
			firstWedge,
			(direction) => direction.wedges,
		);
		expect(wedges.map((wedge) => wedge.wedgeType)).toEqual([
			'crescendo',
			'stop',
		]);
		...
		const wedgeSpanner = new Spanner<Wedge>({
			siblings: wedges,
			isOpen: (wedge) =>
				wedge.wedgeType === 'crescendo' || wedge.wedgeType === 'diminuendo',
