...
export class GuitarProScoreWriter {
	...
	write(document: MDocument): model.Score {
		...
		score.music =
			identification
				...
				.find((creator) => creator.getAttribute('type') === 'composer')?.text ??
			'';
		score.words =
			identification
				?.childrenNamed('creator')
				.find((creator) => creator.getAttribute('type') === 'lyricist')?.text ??
			'';
		score.artist =
			identification
				?.childrenNamed('creator')
				.find((creator) => creator.getAttribute('type') === 'artist')?.text ??
			'';
		...
		for (const part of parts) {
			...
			for (const measure of part.measures) {
				if (
					measure.notes.some(
						(note) => !['1', '2'].slice(0, staffCount).includes(note.staff),
					)
				) {
	...
	private masterBar(measure: Measure): model.MasterBar {
		...
		bar.timeSignatureNumerator = GuitarProValues.integer(
			Number(time?.beats ?? 4),
			'time numerator',
			1,
			64,
		);
		bar.timeSignatureDenominator = GuitarProValues.integer(
		...
		if (
			(bar.timeSignatureDenominator & (bar.timeSignatureDenominator - 1)) !==
			0
		) {
			throw new Error('Guitar Pro time denominator must be a power of two');
		}
		bar.isAnacrusis = measure.isImplicit;
		bar.isRepeatStart = measure.barlines.some(
			(line) => line.repeat === 'forward',
		);
		const repeat = measure.barlines.find((line) => line.repeat === 'backward');
		bar.repeatCount = repeat
			? GuitarProValues.integer(repeat.repeatTimes ?? 2, 'repeat count', 2, 100)
			: 0;
		bar.isDoubleBar = measure.barlines.some(
			(line) => line.barStyle === 'light-light',
		);
		for (const sound of measure.sounds) {
			if (sound.tempo != null) {
				...
				bar.tempoAutomations.push(
					this.codec.model.Automation.buildTempoAutomation(
						false,
						0,
						sound.tempo,
						2,
		...
		for (const direction of measure.directions) {
			...
			for (const metronome of direction.metronomes) {
				...
				const tempo =
					Number(metronome.perMinute) *
					GuitarProValues.beats(duration, { dots: unit.dots });
				bar.tempoAutomations.push(
					this.codec.model.Automation.buildTempoAutomation(false, 0, tempo, 2),
				);
			}
		}
		return bar;
	...
	private writeStaff(part: Part, track: model.Track, staffId: string): void {
		...
		staff.stringTuning.tunings = tunings.map((tuning) =>
			GuitarProValues.integer(tuning.midi, 'tuning pitch', 0, 127),
		);
		if (
			tunings.some((tuning, index) => tuning.line !== tunings.length - index)
		) {
			throw new Error(
				'Guitar Pro tuning lines must be consecutive starting at 1',
			);
		}
		staff.capo = GuitarProValues.integer(
			first.getStaffDetails(staffId)?.capo ?? 0,
			'capo',
			0,
			36,
		);
		...
		const voiceIds = [
			...new Set(
				part.measures.flatMap((measure) =>
					measure.notes
						.filter((note) => note.staff === staffId)
						.map((note) => note.voice),
				),
			),
		];
		...
		for (const measure of part.measures) {
			...
			bar.keySignature = GuitarProValues.integer(
				measure.getKey(staffId)?.fifths ?? 0,
				'key fifths',
				-7,
				7,
			);
			...
			bar.keySignatureType =
				mode === 'minor'
					? this.codec.model.KeySignatureType.Minor
					: this.codec.model.KeySignatureType.Major;
			staff.addBar(bar);
			for (const voiceId of voiceIds) {
				...
				const chords = groupChords(
					measure.notes.filter(
						(note) => note.staff === staffId && note.voice === voiceId,
					),
				);
				let cursor = 0;
	...
	private beat(note: Note): model.Beat {
			...
			(note.child('rest')?.getAttribute('measure') === 'yes' ||
				(note.type === 'whole' &&
					note.measure.notes.filter(
						(sibling) =>
							sibling.voice === note.voice && sibling.staff === note.staff,
					).length === 1));
		const duration = fullRest
			? this.codec.model.Duration.Whole
			: GuitarProValues.durations[note.type ?? ''];
		if (duration == null) {
			throw new Error(
		...
		if (ratio) {
			beat.tupletNumerator = GuitarProValues.integer(
				ratio.actual,
				'tuplet numerator',
				1,
				64,
			);
			beat.tupletDenominator = GuitarProValues.integer(
				ratio.normal,
				'tuplet denominator',
				1,
				64,
		...
		if (note.isGrace) {
			beat.graceType = note.graceSlash
				? this.codec.model.GraceType.BeforeBeat
				: this.codec.model.GraceType.OnBeat;
		} else {
	...
	private note(source: Note, staff: model.Staff): model.Note {
		...
		note.isTieDestination =
			source.ties.some(
				(tie) => tie.tieType === 'stop' || tie.tieType === 'continue',
			) ||
			source
				.childrenNamed('tie')
				.some((tie) => tie.getAttribute('type') === 'stop');
		note.isStaccato = source.articulations.includes('staccato');
		note.accentuated = this.accentuation(source.articulations);
		return note;
	}

	private accentuation(
		articulations: readonly string[],
	): model.AccentuationType {
		if (articulations.includes('strong-accent')) {
			return this.codec.model.AccentuationType.Heavy;
		}
		if (articulations.includes('accent')) {
			return this.codec.model.AccentuationType.Normal;
		}
		return this.codec.model.AccentuationType.None;
	}
	...
	private directions(beat: model.Beat, note: Note): void {
			...
			.flatMap((measure) =>
				measure.directions.filter(
					(direction) =>
						...
						(measure.index < note.measure.index ||
		...
		for (const direction of directions) {
			...
			for (const dynamic of direction.dynamics.flatMap((mark) => mark.marks)) {
				const value =
					this.codec.model.DynamicValue[
						dynamic.toUpperCase() as keyof typeof model.DynamicValue
					];
