...
describe('Lyric', () => {
	it('reads verse, syllabic, and syllable through Note.lyrics', () => {
		const [lyric] = noteWith(
			`<lyric number="2"><syllabic>begin</syllabic><text>love</text></lyric>`,
		).lyrics;
	...
	it('joins elided runs into one syllable', () => {
		const [lyric] = noteWith(
			`<lyric><text>look</text><elision/><text>it</text></lyric>`,
		).lyrics;
	...
	it('exposes every verse attached to the note', () => {
		const note = noteWith(
			`<lyric number="1"><text>one</text></lyric><lyric number="2"><text>two</text></lyric>`,
		);
