...
describe('signature helpers — the backward carry-forward walk', () => {
	...
	it('returns <attributes> in effect nearest-first, scanning back from a note index', () => {
		...
		const fromFirstNote = attributesBackFrom(
			measure1,
			measure1.children.indexOf(firstNote),
		);
		expect(
			fromFirstNote.map((attrs) => attrs.child('divisions')!.text),
		).toEqual(['4']);
	});
	...
	it('bounds the in-measure scan by fromIndex — children.length sees later/mid-measure blocks', () => {
		...
		const fromMeasureEnd = attributesBackFrom(
			measure1,
			measure1.children.length,
		);
		expect(
			fromMeasureEnd.map((attrs) => attrs.child('divisions')!.text),
		).toEqual(['8', '4']);
	});
	...
	it('walks into earlier measures, current measure before earlier ones', () => {
		...
		expect(fromMeasure2.map((attrs) => attrs.child('divisions')!.text)).toEqual(
			['8', '4'],
		);
	});
