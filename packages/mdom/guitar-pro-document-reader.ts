...
export class GuitarProDocumentReader {
  ...
  read(score: model.Score): MDocument {
    if (score.tracks.length === 0 || score.masterBars.length === 0) {
      throw new Error('Guitar Pro score must have at least one track and measure');
    }
    ...
    for (const [type, text] of [
      ...
      if (text) {
        appendValue(identification, 'creator', text).setAttribute('type', type!);
      }
    }
    if (score.copyright) {
    ...
    for (const track of score.tracks) {
      ...
      const scorePart = doc.score.child('part-list')!.childrenNamed('score-part').at(-1)!;
      ...
      appendValue(midi, 'midi-channel', String(track.playbackInfo.primaryChannel + 1));
      ...
      for (const master of score.masterBars) {
        ...
        for (const staff of track.staves) {
          ...
          if (!bar) {
            throw new Error(`Missing Guitar Pro bar ${master.index + 1} in ${track.name}`);
          }
          this.readStaff(staff, bar, measure);
          for (const voice of bar.voices) {
            const beats = voice.index === 0 ? voice.beats : voice.beats.filter((beat) => !beat.isEmpty);
            if (beats.length === 0) {
              continue;
            }
            if (cursor > 0) {
            ...
            for (const beat of beats) {
              cursor += this.readBeat(beat, measure, `${staff.index * 4 + voice.index + 1}`);
            }
          }
        }
        if (master.isRepeatEnd || master.isDoubleBar) {
          measure.addBarline({
            ...
            repeat: master.isRepeatEnd ? { direction: 'backward', times: master.repeatCount } : undefined,
          });
        }
      }
    }
  ...
  private readMaster(master: model.MasterBar, measure: Measure): void {
    GuitarProValues.integer(master.timeSignatureNumerator, 'time numerator', 1, 64);
    GuitarProValues.integer(master.timeSignatureDenominator, 'time denominator', 1, 128);
    measure.setTime({ beats: master.timeSignatureNumerator, beatType: master.timeSignatureDenominator });
    ...
    if (master.alternateEndings || master.directions?.size || master.isFreeTime || master.tripletFeel) {
    ...
    for (const tempo of master.tempoAutomations) {
      ...
      measure.addDirection({ metronome: { beatUnit: 'quarter', perMinute: tempo.value }, tempo: tempo.value });
    }
  }

  private readStaff(staff: model.Staff, bar: model.Bar, measure: Measure): void {
    const staffId = String(staff.index + 1);
    if (staff.transpositionPitch || bar.clefOttava !== this.codec.model.Ottavia.Regular) {
    ...
    measure.setKey({
      ...
      mode: bar.keySignatureType === this.codec.model.KeySignatureType.Minor ? 'minor' : 'major',
      staff: staffId,
    });
  ...
  private readBeat(beat: model.Beat, measure: Measure, voice: string): number {
    ...
    const divisions = Number(measure.child('attributes')!.child('divisions')!.text);
    const type = GuitarProValues.noteType(beat.duration);
    const fullRest = beat.isEmpty || beat.isFullBarRest;
    const duration =
      beat.graceType !== this.codec.model.GraceType.None
        ? 0
        : fullRest
          ? (beat.voice.bar.masterBar.calculateDuration() / 960) * divisions
    ...
    if (!Number.isInteger(duration)) {
      throw new Error(`Cannot represent Guitar Pro duration in measure ${measure.number}`);
    }
    if (
      beat.hasWhammyBar ||
      beat.vibrato ||
      ...
      this.values.unsupported(`beat effects or annotations in measure ${measure.number}`);
    }
    ...
    if (dynamic && beat.graceType === this.codec.model.GraceType.None) {
      measure.addDirection({ dynamics: dynamic, staff: String(staff.index + 1) });
    }
    if (beat.text) {
      measure.addDirection({ words: beat.text, staff: String(staff.index + 1) });
    }
    ...
    for (const [index, source] of notes.entries()) {
      ...
      if (source) {
        note.append(
          buildPitch(GuitarProValues.pitch(source.realValueWithoutHarmonic, { flats: beat.voice.bar.keySignature < 0 }))
        );
      } else {
  ...
  private readNote(source: model.Note, note: Note, staff: model.Staff): void {
    ...
    if (source.isStringed) {
      note.setStringFret({ string: staff.tuning.length - source.string + 1, fret: source.fret });
    }
    if (source.isStaccato) {
      note.addArticulation('staccato');
    }
    if (source.accentuated !== this.codec.model.AccentuationType.None) {
      note.addArticulation(source.accentuated === this.codec.model.AccentuationType.Heavy ? 'strong-accent' : 'accent');
    }
        ...
        for (const beat of voice.beats) {
          if (beat.hasTuplet) {
            ...
            if (!Number.isSafeInteger(divisions) || divisions > 100_000_000) {
              throw new Error('Guitar Pro tuplet ratios require too many divisions');
            }
          }
        }
      }
