...
describe('Barline', () => {
	...
	it('exposes both edges when a measure opens and closes a repeat', () => {
		...
		expect(measure.barlines.map((barline) => barline.location)).toEqual([
			'left',
			'right',
		]);
		expect(measure.barlines.map((barline) => barline.repeat)).toEqual([
			'forward',
			'backward',
		]);
	});
