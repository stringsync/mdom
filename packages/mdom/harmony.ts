...
export class Harmony extends MElement {
	...
	get kind(): { value: HarmonyKindValue; text: string | null } | null {
		...
		return {
			value: value as HarmonyKindValue,
			text: kind!.getAttribute('text'),
		};
	}
	...
	setFrame(spec: FrameSpec): Frame {
		...
		const tail =
			this.childrenOfType(MElement).find((child) =>
				AFTER_FRAME.has(child.tag),
			) ?? null;
...
export function buildHarmony(spec: HarmonySpec): Harmony {
	...
	const kind =
		typeof spec.kind === 'string'
			? { value: spec.kind, text: undefined }
			: spec.kind;
