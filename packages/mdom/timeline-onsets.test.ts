...
const beatsOf = (measure: Measure): (number | null)[] =>
	measure.notes.map((eachNote) => eachNote.measureBeat);
...
describe('onset fold — backup/forward/chord/grace', () => {
	it('advances the cursor by the duration of each note', () => {
		const measure = measureFrom(
			`${noteXml('C', 4, 4)}${noteXml('D', 4, 8)}${noteXml('E', 4, 4)}`,
		);
	...
	it('sits a <grace/> note at the cursor, stealing no time', () => {
		const measure = measureFrom(
			`${noteXml('C', 5, null, '<grace/>')}${noteXml('D', 4, 4)}${noteXml('E', 4, 4)}`,
		);
	...
	it('opens a gap on <forward>', () => {
		const measure = measureFrom(
			`${noteXml('C', 4, 4)}<forward><duration>8</duration></forward>${noteXml('D', 4, 4)}`,
		);
	...
	it('lets a zero-duration element pass without moving the cursor', () => {
		...
		const measure = measureFrom(
			`${noteXml('C', 4, 4)}${direction}${noteXml('D', 4, 4)}`,
		);
