...
describe('Wedge — a direction-attached hairpin', () => {
  ...
  it('reads the opener type and its host <direction>, not a note', () => {
    ...
    const crescendo = part.getMeasure('1')!.directions.flatMap((direction) => direction.wedges)[0]!;
  ...
  it('opens on a diminuendo just as it does on a crescendo', () => {
    ...
    const diminuendo = part.getMeasure('2')!.directions.flatMap((direction) => direction.wedges)[0]!;
