...
describe('Guitar Pro round trips', () => {
  ...
  it('keeps separate voices aligned with rests filling explicit gaps', async () => {
    measure.getOrCreateVoice('high').addNote({ step: 'C', octave: 5, type: 'half', onset: 2 });
    measure.getOrCreateVoice('low').addNote({ step: 'C', octave: 3, type: 'whole' });
    const result = await parser.parseFromBytes(await serializer.serializeToBytes(document));

    expect(
      result.score.parts[0]!.measures[0]!.notes.map((note) => [note.voice, note.measureBeat, note.beats, note.isRest])
    ).toEqual([
      ['1', 0, 2, true],
      ['1', 2, 2, false],
      ['2', 0, 4, false],
  ...
  it('keeps treble and bass staves in one part', async () => {
    ...
    measure.getOrCreateVoice('1').addNote({ step: 'C', octave: 5, type: 'whole' });
    measure.getOrCreateVoice('2', { staff: '2' }).addNote({ step: 'C', octave: 3, type: 'whole' });
    const result = await parser.parseFromBlob(await serializer.serializeToBlob(document));
    ...
    expect(restored.notes.map((note) => [note.staff, note.pitch?.octave, note.measureBeat])).toEqual([
      ['1', 5, 0],
      ['2', 3, 0],
    ]);
    expect(schemaErrors(result)).toEqual([]);
  ...
  it('keeps tied notes connected across measures', async () => {
    const first = measure.getOrCreateVoice('1').addNote({ step: 'C', octave: 4, type: 'whole' });
    ...
    expect(measures[0]!.notes[0]!.ties[0]!.partner?.note).toBe(measures[1]!.notes[0]);
    expect(measures[1]!.notes[0]!.ties[0]!.tieType).toBe('stop');
    expect(schemaErrors(result)).toEqual([]);
  });

  it('keeps grace notes off the measured timeline', async () => {
    const grace = measure.getOrCreateVoice('1').addNote({ step: 'D', octave: 4, type: 'eighth' });
    ...
    expect(notes.map((note) => [note.isGrace, note.duration, note.measureBeat])).toEqual([
      [true, null, 0],
      [false, 1024, 0],
    ]);
    expect(schemaErrors(result)).toEqual([]);
  ...
  it('keeps whole-measure rests in three-four time', async () => {
    ...
    expect(result.score.parts[0]!.measures[0]!.notes[0]).toMatchObject({ isRest: true, beats: 3 });
    expect(schemaErrors(result)).toEqual([]);
  });

  it('keeps tempo, dynamics and simple articulations', async () => {
    measure.addDirection({ tempo: 84, metronome: { beatUnit: 'quarter', perMinute: 84 }, dynamics: 'p' });
    ...
    expect(
      restored.directions.flatMap((direction) => direction.dynamics.flatMap((dynamic) => dynamic.marks))
    ).toContain('p');
  ...
  it('does not retain tracks or notes between conversions', async () => {
    measure.getOrCreateVoice('1').addNote({ step: 'C', octave: 4, type: 'whole' });
    const first = await parser.parseFromBlob(await serializer.serializeToBlob(document));
    const second = MDocument.empty();
    second.score.addPart({ name: 'Solo' }).addMeasure().getOrCreateVoice('1').addRest({ type: 'whole' });
    const result = await parser.parseFromBlob(await serializer.serializeToBlob(second));
    ...
    expect(result.score.parts[0]!.measures[0]!.notes.map((note) => note.isRest)).toEqual([true]);
  });
  ...
  it('carries dynamics across notes and barlines', async () => {
    ...
    document.score.parts[0]!.addMeasure().getOrCreateVoice('1').addNote({ step: 'E', octave: 4, type: 'whole' });
    const result = await parser.parseFromBlob(await serializer.serializeToBlob(document));

    expect(
      result.score.parts[0]!.measures.flatMap((bar) =>
        bar.directions.flatMap((direction) => direction.dynamics.flatMap((mark) => mark.marks))
      )
    ).toEqual(['p', 'p', 'p']);
  });
  ...
  it('keeps text at its original beat without repeating it later', async () => {
    ...
    expect(result.score.parts[0]!.measures[0]!.directions.flatMap((direction) => direction.words)).toEqual([
      'With feeling',
    ]);
  });

  it('converts a dotted metronome beat to quarter-note tempo without a sound element', async () => {
    measure.addDirection({ metronome: { beatUnit: 'quarter', dots: 1, perMinute: 80 } });
    measure.getOrCreateVoice('1').addRest({ type: 'whole' });
    const result = await parser.parseFromBlob(await serializer.serializeToBlob(document));

    expect(result.score.parts[0]!.measures[0]!.sounds.map((sound) => sound.tempo)).toEqual([120]);
  });

  it('keeps a voice assigned to the same slot when another voice is silent', async () => {
    measure.getOrCreateVoice('upper').addNote({ step: 'C', octave: 5, type: 'whole' });
    measure.getOrCreateVoice('lower').addNote({ step: 'C', octave: 3, type: 'whole' });
    document.score.parts[0]!.addMeasure().getOrCreateVoice('lower').addNote({ step: 'D', octave: 3, type: 'whole' });
    const result = await parser.parseFromBlob(await serializer.serializeToBlob(document));

    expect(
      result.score.parts[0]!.measures[1]!.notes.map((note) => [note.voice, note.isRest, note.measureBeat])
    ).toEqual([
      ['1', true, 0],
      ['2', false, 0],
    ]);
