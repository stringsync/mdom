...
describe('MElement — positional mutation', () => {
	...
	it('inserts before a reference child', () => {
		...
		expect(parent.childrenOfType(MElement).map((child) => child.tag)).toEqual([
			'pitch',
			'grace',
			'duration',
		]);
	...
	it('throws when the reference is not a child', () => {
		const { parent } = build();
		expect(() =>
			parent.insertBefore(new MElement('dot'), new MElement('stray')),
		).toThrow('insertBefore reference is not a child');
	});
	...
	it('throws when the replacement target is not a child', () => {
		const { parent } = build();
		expect(() =>
			parent.replaceChild(new MElement('stray'), new MElement('rest')),
		).toThrow('replaceChild target is not a child');
	});
