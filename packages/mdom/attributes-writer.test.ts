...
const blocks = (measure: Measure): string[][] =>
	measure
		...
		.map((attrs) => attrs.childrenOfType(MElement).map((child) => child.tag));
...
describe('measure — writing <attributes>', () => {
	...
	it('declares a stave count, so a grand staff can be built', () => {
		...
		measure
			.getOrCreateVoice('1', { staff: '1' })
			.addNote({ step: 'C', octave: 5, type: 'whole' });
		measure
			.getOrCreateVoice('2', { staff: '2' })
			.addNote({ step: 'C', octave: 3, type: 'whole', onset: 0 });
	...
	it('writes a mid-measure change when asked for one by onset', () => {
		...
		expect(
			measure.clefChanges().map((change) => [change.beat, change.clef.sign]),
		).toEqual([[2, 'F']]);
	...
	it('rewinds to an earlier onset with a <backup>, the grand-staff spelling', () => {
		...
		expect(
			measure.children.map((node) =>
				node instanceof MElement ? node.tag : 'text',
			),
		).toEqual(['attributes', 'note', 'backup', 'attributes', 'note']);
		expect(
			measure.clefChanges('2').map((change) => [change.beat, change.clef.sign]),
		).toEqual([[0, 'C']]);
		expect(schemaErrors(doc)).toEqual([]);
	});
