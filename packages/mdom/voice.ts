...
export type { DurationSpec, NoteSpec, NoteType, PitchSpec } from './note';
...
export class Voice {
	...
	private open(spec: DurationSpec): number {
		...
		const target =
			spec.onset != null
				? spec.onset * divisions
				: voiceEnd(this.measure, this.id);
	...
	private divisions(): number {
		const existing = divisionsBackFrom(
			this.measure,
			this.measure.children.length,
		);
		if (existing != null) {
			return existing;
		}
